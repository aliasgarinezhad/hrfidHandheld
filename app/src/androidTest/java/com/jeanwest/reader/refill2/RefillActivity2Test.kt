...
@ExperimentalCoilApi
class RefillActivity2Test {

    @get:Rule
    val activity = createAndroidComposeRule<RefillActivity2>()
    ...
    fun test1() {
        ...
        assert(activity.activity.inputBarcodes.size > 5)
        ...
        for (i in 0 until 3) {
            ...
            activity.onAllNodesWithTag("items")[i].apply {
                ...
                assertTextContains("فروشگاه: " + activity.activity.refillProducts[i].storeNumber)
        ...
        barcodeArrayScan(5, scannedProducts)

        restart()

        activity.onNodeWithText("پیدا شده: " + 5)
            .assertExists()
        activity.onNodeWithText("خطی: " + activity.activity.inputBarcodes.size)
            .assertExists()
