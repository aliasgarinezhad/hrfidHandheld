...
class CheckInActivityTest2 {
    ...
    fun test() {
        ...
        checkInActivity.onNodeWithText("اضافی: 0").assertExists()
        checkInActivity.onNodeWithText("اسکن: $inputProductsNumber").assertExists()
    ...
    private fun epcScan(withDifference: Boolean, rfidMap: MutableMap<Long, Int>) {
        ...
        rfidMap.forEach {
            ...
            if (withDifference) {
                checkInActivity.activity.inputEpcTable.forEach { it1 ->
                    val uhfTagInfo = UHFTAGInfo()
                    uhfTagInfo.epc = it1.key
    ...
    private fun barcodeScanNotEpcs() {

        val conflictCopy = mutableListOf<Product>()
        conflictCopy.addAll(checkInActivity.activity.conflicts)

        conflictCopy.forEach {
            if (it.scan == "کسری") {
                repeat(it.matchedNumber) { _ ->
