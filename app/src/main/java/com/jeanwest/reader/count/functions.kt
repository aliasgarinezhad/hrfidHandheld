...
fun setRFEpcMode(rf: RFIDWithUHFUART, context: Context): Boolean {
    for (i in 0..11) {
        if (rf.setEPCMode()) {
            return true
        }
...
fun rfInit(rf: RFIDWithUHFUART, context: Context) {
    ...
    setRFEpcMode(rf, context)
}

fun setRFPower(context: Context, rf: RFIDWithUHFUART, power: Int): Boolean {
    if (rf.power != power) {
